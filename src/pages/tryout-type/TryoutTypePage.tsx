...
import { useDeleteDataMutation, useFetchDataQuery } from "../../services/tryoutTypeApi";
import { PlusIcon } from "../../assets/icons";
import TryoutTypeTable from "./TryoutTypeTable";
import { TryoutTypeModal } from "./TryoutTypeModal";
import { useNavigate } from "react-router-dom";

const LIMITS = [5, 10, 15, 20, 25, 50, 75, 100];

export default function TryoutTypePage(){
  ...
  const eventDeleteHandler = async (data: any): Promise<void> => {
    if (
      ...
      try {
        const response = await deleteData(data?.type_id).unwrap();
  ...
  return (
    <>
      <PageMeta
        ...
        description="This is master tryout type on admin panel"
      />

      <BreadCrumb pageTitle="TIPE TRYOUT" />
      <div className="space-y-6">
        <ComponentCard title="MASTER DATA TIPE TRYOUT">
          {/* Top Controls */}
