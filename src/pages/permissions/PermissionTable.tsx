...
import { useDeletePermissionMutation } from "../../services/permissionApi";
...
const tableHeaders: TableHeaders[] = [
  ...
  { id: 3, title: "Nomor Urut", name: "order_number" },
...
export default function PermissionTable({
  ...
  const eventDeleteHandler = async (data: any): Promise<void> => {
    if (window.confirm(`Apakah kamu yakin ingin menghapus permission "${data.name}" ini?`)) {
      try {
        const response = await deletePermission(data.permission_id).unwrap();
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {records.length > 0 ? (
                  records.map((record, index) => (
                    <TableRow key={record.permission_id || index}>
                      ...
                      <TableCell className="px-4 py-3 text-start text-theme-sm text-slate-700 dark:text-slate-600">
                        <div className="flex gap-2">
                          <button
                            onClick={() => onEdit(record.permission_id)}
